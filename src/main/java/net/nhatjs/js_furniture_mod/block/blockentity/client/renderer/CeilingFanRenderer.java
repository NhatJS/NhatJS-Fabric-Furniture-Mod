...
public class CeilingFanRenderer implements BlockEntityRenderer<CeilingFanBlockEntity> {
    ...
    public void render(CeilingFanBlockEntity be, float tickDelta,
        ...
        BakedModel blades = mc.getBakedModelManager().getModel(NhatJSFurnitureModClient.CEILING_FAN_BLADES_ID);

        if (blades == null || blades == mc.getBakedModelManager().getMissingBlockModel()) {
            return;
        }
        ...
        VertexConsumer vc = vcp.getBuffer(RenderLayer.getCutoutMipped()); // 1.21
        brm.getModelRenderer().render(w, blades, be.getCachedState(), be.getPos(), ms, vc, false, w.getRandom(), 42L, overlay);
        ms.pop();
    }
}