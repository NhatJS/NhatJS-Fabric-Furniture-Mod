...
public class StudioLightBlock extends FurnitureHorizontalBlock implements BlockEntityProvider {
    public static final BooleanProperty TURN_ON = BooleanProperty.of("turn_on");

    public StudioLightBlock(Settings settings) {
        super(settings.luminance(st -> st.get(TURN_ON) ? 15 : 0));
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            if (!(entity instanceof LampBlockEntity studioLight)) return ActionResult.PASS;
